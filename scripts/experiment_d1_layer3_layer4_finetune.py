...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Experiment D1: layer3 + layer4 ResNet-50 fine-tuning "
...
def main():
    ...
    print("=" * 70)
    print("DERMASENSE EXPERIMENT D1")
    print(
        "LAYER3 + LAYER4 RESNET-50 FINE-TUNING: "
        "ISIC → PAD-UFES"
    )
    print("=" * 70)
    ...
    if layer3 is None:
        raise RuntimeError(
            "Could not identify ResNet-50 layer3 "
            "inside the backbone."
        )
    ...
    for parameter in layer3.parameters():
    ...
    if trainable_backbone != (
        trainable_layer3 + trainable_layer4
    ):
        raise RuntimeError(
            "Unexpected trainable backbone parameters. "
            "Only layer3 and layer4 should be trainable."
        )

    if trainable_layer3 == 0:
        raise RuntimeError(
            "Layer3 must be trainable."
        )
    ...
    if best_state is None:
        raise RuntimeError(
            "No best D1 checkpoint was produced."
        )
    ...
    print()
    print("=" * 70)
    print("BEST D1 VALIDATION RESULT")
    print("=" * 70)
    ...
    # ------------------------------------------------------------
    # SAVE BEST D1 CHECKPOINT
    # ------------------------------------------------------------

    d1_checkpoint_path = Path(
        "checkpoints/"
        "pad_ufes_d1_layer3_layer4_best.pt"
    )

    d1_checkpoint_path.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    torch.save(
        {
            ...
            "experiment": "D1",
            "architecture": (
                "resnet50_layer3_layer4_finetune"
            ),
            "dataset_id": "pad_ufes",
            "seed": args.seed,
            "classes": PAD_CLASSES,
